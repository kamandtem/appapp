...
import { shareBlob } from '../services/fileActions';
...
export const PdfReader: React.FC<{ document: PdfDocument; onClose: () => void }> = ({ document, onClose }) => {
  ...
  const share = async () => {
    ...
    try {
      ...
      setFeedback(result === 'shared' ? 'فایل PDF برای ارسال یا ذخیره آماده شد.' : 'فایل PDF دانلود شد.');
    } catch (error) {
      if ((error as DOMException)?.name === 'AbortError') { setFeedback('ارسال لغو شد.'); return; }
      setFeedback('خروجی PDF انجام نشد؛ دوباره امتحان کن.');
    }
  };
  const save = async () => {
    setFeedback('در حال آماده‌سازی فایل PDF...');
    try {
      await shareBlob(document.blob, document.fileName, `ذخیره ${document.fileName}`);
      setFeedback('از پنجره بازشده محل ذخیره فایل را انتخاب کن.');
    } catch (error) {
      if ((error as DOMException)?.name === 'AbortError') { setFeedback('ذخیره لغو شد.'); return; }
      setFeedback('ذخیره PDF انجام نشد؛ دوباره امتحان کن.');
    }
  };
  return <section className="card overflow-hidden" aria-label="نمایش PDF">
    <header className="flex items-center gap-2 p-3 border-b border-line bg-surface sticky top-0 z-10">
      ...
      <button type="button" onClick={save} className="icon-button" aria-label="ذخیره PDF"><Download className="w-4 h-4" /></button>
