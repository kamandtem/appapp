...
import { STUDIO_SERVICES, STUDIO_CAMERAS, priceForLine } from '../data/studioCatalog';
...
function initialCeremonyLines(project: OfficeProject, profile: StudioProfile | null): Line[] {
  const lines = (project.ceremonyInvoice?.items || []).map(item => ({ ...item, price: item.price || priceForLine(item.name, profile) }));
...
export const OfficeProjectEditor: React.FC<Props> = ({ project, profile, onSave, onClose }) => {
  ...
  const [formalityLines, setFormalityLines] = useState<Line[]>(() => (project.formalityInvoice?.items || []).map(item => ({ ...item, price: item.price || priceForLine(item.name, profile) }))); 
  ...
  const addCustomService = () => {
    ...
    setCeremonyLines(v => [...v, { name: value, count: 1, price: priceForLine(value, profile) }]);
  ...
  return <div className="space-y-4 pb-8">
    ...
    {step === 1 && <section className="card p-4 space-y-4"><div className="flex items-center gap-2"><span className="w-9 h-9 rounded-xl flex items-center justify-center bg-[var(--color-surface2)] text-gold"><Users className="w-4 h-4" /></span><div><b>پروفایل آتلیه</b><p className="text-[10px] text-muted">نام و قیمت‌های پایه از پروفایل استودیو خوانده می‌شوند.</p></div></div><Field label="نام پروژه" value={name} setValue={setName} placeholder="مثلاً عروسی علی و سارا" /><div className="grid grid-cols-2 gap-2"><Field label="نام داماد" value={groomName} setValue={setGroomName} placeholder="نام و نام خانوادگی" autoComplete="name" /><Field label="نام عروس" value={brideName} setValue={setBrideName} placeholder="نام و نام خانوادگی" autoComplete="name" /><Field label="کد ملی داماد" value={groomNationalId} setValue={setGroomNationalId} inputMode="numeric" pattern="[0-9۰-۹]*" maxLength={10} /><Field label="کد ملی عروس" value={brideNationalId} setValue={setBrideNationalId} inputMode="numeric" pattern="[0-9۰-۹]*" maxLength={10} /></div><div className="grid grid-cols-2 gap-2"><Field label="شماره تماس اول" value={clientPhone} setValue={setClientPhone} inputMode="tel" autoComplete="tel" /><Field label="شماره تماس دوم" value={secondaryPhone} setValue={setSecondaryPhone} inputMode="tel" /></div><Field label="نشانی" value={customerAddress} setValue={setCustomerAddress} placeholder="نشانی کامل زوج" /><ChoiceField label="نوع مراسم" options={['عروسی', 'عقد', 'عقد و عروسی']} value={ceremonyType} onChange={value => setCeremonyType(value as typeof ceremonyType)} /><div className="office-contract-notes office-contract-notes-lower"><span className="label">توضیحات و سفارش‌های خاص</span><textarea value={contractNotes} onChange={e => setContractNotes(e.target.value)} className="field min-h-24" placeholder="موارد توافق‌شده با زوج..." /></div></section>}
    {step === 2 && <section className="space-y-3"><Toggle title="مراسم" icon={Film} active={ceremonyOn} onClick={() => setCeremonyOn(!ceremonyOn)} />{ceremonyOn && <div className="card p-4 space-y-4"><Field label="محل مراسم" value={ceremonyLocation} setValue={setCeremonyLocation} placeholder="تالار، باغ یا عمارت" /><div><span className="label">تاریخ مراسم</span><JalaliDatePicker value={ceremonyDate} onChange={setCeremonyDate} /></div><div className="grid grid-cols-2 gap-2"><TimeField label="ساعت شروع" value={startTime} onChange={setStartTime} /><TimeField label="ساعت پایان" value={endTime} onChange={setEndTime} /></div><MoneyField label="مبلغ هر ساعت اضافه، تومن" value={extraHourPrice} onChange={setExtraHourPrice} /><ChoiceList title="خدمات موردنظر زوج" values={SERVICES} selected={ceremonyServices} onToggle={toggleService} /><div className="rounded-2xl border border-line p-3 space-y-2"><span className="label">خدمت سفارشی</span><div className="flex gap-2"><input value={customServiceName} onChange={e => setCustomServiceName(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') { e.preventDefault(); addCustomService(); } }} className="field flex-1" placeholder="نام خدمت را بنویس" /><button type="button" onClick={addCustomService} className="btn btn-primary px-4"><Plus className="w-4 h-4" />افزودن</button></div>{customServices.map(item => <div key={item.id} className="flex items-center gap-2 rounded-xl bg-surface2 px-3 py-2"><Check className="w-4 h-4 text-[var(--color-teal)]" /><b className="flex-1 text-[12px]">{item.name}</b><button type="button" onClick={() => removeCustomService(item.id, item.name)} className="w-9 h-9 grid place-items-center text-rose"><Trash2 className="w-4 h-4" /></button></div>)}</div><div><span className="label flex items-center gap-1"><Camera className="w-3.5 h-3.5 text-gold" />تجهیزات لازم</span><div className="grid grid-cols-2 gap-2">{CAMERAS.map(camera => { const on = (ceremonyCameras[camera] || 0) > 0; return <button key={camera} type="button" onClick={() => toggleCamera(camera)} className={`p-3 rounded-2xl border text-right ${on ? 'border-[var(--color-teal)] bg-[color-mix(in_srgb,var(--color-teal)_10%,transparent)]' : 'border-line'}`}><span className="flex items-center gap-2 text-[12px] font-bold"><span className={`w-5 h-5 rounded-full grid place-items-center ${on ? 'bg-[var(--color-teal)] text-bg' : 'bg-surface2'}`}>{on && <Check className="w-3 h-3" />}</span>{camera}</span>{on && <span className="block text-[10px] text-muted mt-1">{ceremonyCameras[camera]} دستگاه</span>}</button>; })}</div></div></div>}
      <Toggle title="فرمالیته" icon={Clock} active={formalityOn} onClick={() => setFormalityOn(!formalityOn)} />{formalityOn && <div className="card p-4 space-y-4"><Field label="محل فرمالیته" value={formalityLocation} setValue={setFormalityLocation} /><div><span className="label">تاریخ فرمالیته</span><JalaliDatePicker value={formalityDate} onChange={setFormalityDate} /></div><ChoiceField label="نوع کلیپ" options={LOCATIONS} value={clipType} onChange={value => setClipType(value as LocationTypeFormatted)} /><ChoiceField label="تم درخواستی" options={THEMES} value={theme} onChange={value => setTheme(value as ThemeType)} /></div>}</section>}
    {step === 3 && <section className="space-y-3"><InvoiceEditor title="فاکتور مراسم" items={ceremonyLines} deposit={ceremonyDeposit} profile={profile} onDeposit={setCeremonyDeposit} onChange={setCeremonyLines} /><InvoiceEditor title="فاکتور فرمالیته" items={formalityLines} deposit={formalityDeposit} profile={profile} onDeposit={setFormalityDeposit} onChange={setFormalityLines} /><div className="card p-4 flex items-center justify-between"><span className="font-bold">جمع نهایی</span><strong className="text-[18px] text-gold">{formatMoney(grandTotal)} تومن</strong></div></section>}
...
const Field: React.FC<{ label: string; value: string; setValue: (v: string) => void; placeholder?: string; inputMode?: React.HTMLAttributes<HTMLInputElement>['inputMode']; pattern?: string; maxLength?: number; autoComplete?: string }> = ({ label, value, setValue, placeholder, inputMode = 'text', pattern, maxLength, autoComplete }) => <label className="form-field-shell"><span className="label">{label}</span><input value={value} onChange={e => setValue(e.target.value)} className="field" placeholder={placeholder} inputMode={inputMode} pattern={pattern} maxLength={maxLength} autoComplete={autoComplete} /></label>;

const ChoiceField: React.FC<{ label: string; options: readonly string[]; value: string; onChange: (v: string) => void }> = ({ label, options, value, onChange }) => <fieldset className="choice-field"><legend>{label}</legend><div className="choice-field-options">{options.map(option => <button type="button" key={option} onClick={() => onChange(option)} className={value === option ? 'selected' : ''}><span className="choice-dot">{value === option && <Check className="w-3.5 h-3.5" />}</span>{option}</button>)}</div></fieldset>;
...
const InvoiceEditor: React.FC<{ title: string; items: Line[]; deposit: number; profile: StudioProfile | null; onDeposit: (n: number) => void; onChange: (items: Line[]) => void }> = ({ title, items, deposit, profile, onDeposit, onChange }) => {
  const addCatalogItem = (name: string) => { if (!items.some(item => item.name === name)) onChange([...items, { name, count: 1, price: priceForLine(name, profile) }]); };
  return <div className="card p-4 space-y-3"><h3 className="font-extrabold flex items-center gap-2"><Receipt className="w-4 h-4 text-gold" />{title}</h3><div className="invoice-catalog"><span>افزودن از لیست خدمات و تجهیزات</span><div>{[...SERVICES, ...CAMERAS.map(camera => `دوربین/${camera}`)].map(name => <button type="button" key={name} disabled={items.some(item => item.name === name)} onClick={() => addCatalogItem(name)}>{name.replace('دوربین/','')}</button>)}</div></div>{items.map((item, i) => <div key={i} className="invoice-line"><label className="invoice-description"><span className="invoice-field-label">شرح خدمت</span><input value={item.name} onChange={e => { const n = [...items]; n[i] = { ...n[i], name: e.target.value, price: item.name === e.target.value ? item.price : priceForLine(e.target.value, profile) }; onChange(n); }} className="field invoice-field text-[11px]" placeholder="شرح خدمت یا مورد سفارشی" /></label><button type="button" onClick={() => onChange(items.filter((_, j) => j !== i))} className="w-9 h-9 grid place-items-center text-rose self-end" aria-label="حذف مورد"><Trash2 className="w-4 h-4" /></button><div className="invoice-price-row"><Counter value={item.count} onChange={v => { const n = [...items]; n[i] = { ...n[i], count: v }; onChange(n); }} /><label className="flex-1"><span className="invoice-field-label">قیمت واحد، تومن</span><input type="text" inputMode="numeric" value={item.price ? formatMoney(item.price) : ''} onChange={e => { const n = [...items]; n[i] = { ...n[i], price: parseMoney(e.target.value) }; onChange(n); }} className="field invoice-field text-[11px] tabular-nums" placeholder="۰" /></label></div><span className="text-[11px] font-bold self-end text-muted pb-3">{formatMoney(item.count * item.price)} تومن</span></div>)}<button type="button" onClick={() => onChange([...items, { name: '', count: 1, price: 0 }])} className="btn btn-ghost w-full"><Plus className="w-4 h-4 text-gold" />افزودن مورد سفارشی</button><MoneyField label="بیعانه، تومن" value={deposit} onChange={onDeposit} /><div className="flex justify-between font-extrabold text-gold"><span>جمع</span><span>{formatMoney(total(items))} تومن</span></div></div>;
};
