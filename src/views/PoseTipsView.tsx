...
const PoseTipDetail: React.FC<{ pose: PoseTip; onBack: () => void }> = ({ pose, onBack }) => <article className="space-y-6" dir="rtl">
  ...
  <DetailSection icon={Check} title="مراحل اجرا" englishTitle="How to pose"><ol className="space-y-3" dir="ltr">{pose.steps.map((step, index) => <li key={step} className="flex items-start gap-3 text-left"><span className="grid h-7 w-7 shrink-0 place-items-center rounded-full bg-olive text-[10px] font-black text-paper">{index + 1}</span><p className="pt-0.5 text-[12px] leading-6 text-muted">{step}</p></li>)}</ol></DetailSection>
  <DetailSection icon={Camera} title="تنظیم دوربین" englishTitle="Camera setup"><dl className="grid grid-cols-2 gap-x-4 gap-y-5 text-left" dir="ltr"><CameraFact label="Angle" value={pose.camera_tips.angle} /><CameraFact label="Framing" value={pose.camera_tips.framing} /><CameraFact label="Lens" value={pose.camera_tips.lens} /><CameraFact label="Light" value={pose.camera_tips.lighting} /></dl></DetailSection>
  <DetailSection icon={CircleAlert} title="اشتباهات رایج" englishTitle="Common mistakes" tone="rose"><ul className="space-y-2.5 text-left" dir="ltr">{pose.common_mistakes.map(mistake => <li key={mistake} className="flex items-start gap-2 text-[12px] leading-6 text-muted"><X className="mt-1 h-3.5 w-3.5 shrink-0 text-rose" />{mistake}</li>)}</ul></DetailSection>
  <div className="flex flex-wrap gap-2">{pose.tags.map(tag => <span key={tag} className="pill" dir="rtl">{tag}</span>)}</div>
</article>;

const DetailSection: React.FC<{ icon: React.ElementType; title: string; englishTitle: string; tone?: 'olive' | 'rose'; children: React.ReactNode }> = ({ icon: Icon, title, englishTitle, tone = 'olive', children }) => <section className="rounded-[24px] border border-line bg-surface p-4 sm:p-5"><header className="mb-5 flex items-center gap-3"><span className={`grid h-10 w-10 place-items-center rounded-2xl ${tone === 'rose' ? 'text-rose' : 'text-olive'}`} style={{ background: tone === 'rose' ? 'color-mix(in oklch, var(--color-rose) 10%, var(--color-surface))' : 'color-mix(in oklch, var(--color-olive) 10%, var(--color-surface))' }}><Icon className="h-5 w-5" /></span><div><h2 className="text-[14px] font-black">{title}</h2><p className="mt-0.5 text-left text-[9px] text-faint" dir="ltr">{englishTitle}</p></div></header>{children}</section>;
const CameraFact: React.FC<{ label: string; value?: string }> = ({ label, value }) => <div><dt className="text-[9px] font-black uppercase tracking-[.08em] text-olive">{label}</dt><dd className="mt-1.5 text-[12px] leading-5 text-muted">{value || 'Not specified'}</dd></div>;
